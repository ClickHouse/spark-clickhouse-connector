...
1. **Write Operations**:
   - `ClickHouseWriter.write()` - Main write entry point
   - `ClickHouseArrowStreamWriter.writeRow()` - Arrow format writer
   - `ClickHouseJsonEachRowWriter.writeRow()` - JSON format writer

2. **Read Operations**:
   - `ClickHouseReader.next()` - Reading records
   - `ClickHouseBinaryReader` - Binary format reader
   - `ClickHouseJsonReader` - JSON format reader

3. **Catalog Operations**:
   - `ClickHouseCatalog.createTable()` - Table creation
   - `ClickHouseCatalog.loadTable()` - Table metadata loading
   - `ClickHouseHelper.queryTableSpec()` - Table spec queries
...
If you encounter LZ4 decompression errors, disable compression:
```scala
...
- Modify the examples to test specific scenarios
- Add custom transformations to the streaming pipeline
- Test different write formats (arrow vs json)
- Experiment with partitioning and ordering strategies
