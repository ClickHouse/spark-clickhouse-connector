...
### Fixed
- Partition listings are unioned across a table's replicas ([#TBD](https://github.com/ClickHouse/spark-clickhouse-connector/pull/TBD)). A scan plans one input partition per partition reported by `system.parts`, each filtering on `_partition_id`, so a listing that omits a partition prunes it out of the scan and silently drops its rows. `system.parts` reflects only the server answering the query, and on an eventually consistent service such as ClickHouse Cloud — where each operation opens its own connection and may reach a different replica — it lags a recent write: two listings of the same table one second apart were observed returning 3 and 4 partitions, and the read built from the shorter one returned 3 of 4 rows. Whole-table listings now read `clusterAllReplicas(<cluster>, system.parts)`, deduplicated by part name so a part reported by several replicas is counted once, which closes the window to a write landing in the same instant as the listing. The cluster is set by the new `spark.clickhouse.read.partitionListing.cluster` (default `default`, empty to disable); unavailable replicas are skipped, and any failure — an unknown cluster, or a table absent from other members — falls back to the answering server's own view, which is what the connector read before. Per-shard listings under `spark.clickhouse.read.distributed.convertLocal` are left alone, since the union would report every shard's partitions to each shard. Applies to all Spark profiles (3.3/3.4/3.5/4.0).
