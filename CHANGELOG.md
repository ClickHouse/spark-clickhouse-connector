...
### Fixed
- Tables containing columns with unsupported ClickHouse types (e.g. `AggregateFunction`, `Point`) no longer fail every query and write during analysis ([#457](https://github.com/ClickHouse/spark-clickhouse-connector/issues/457)). Such columns are now excluded from the Spark table schema with a warning, so operations touching only supported columns succeed; explicitly referencing an excluded column fails with Spark's unresolved-column error. The excluded columns are listed in the `unsupported.columns` table property (visible via `SHOW TBLPROPERTIES` / `DESCRIBE TABLE EXTENDED`). Applies to all Spark profiles (3.3/3.4/3.5/4.0).
