...
### Added
- Additional write metrics ([#558](https://github.com/ClickHouse/spark-clickhouse-connector/pull/558)): number of flushes (batch inserts), min/max flushed batch size (rows), and connections opened, across all Spark profiles (3.3/3.4/3.5/4.0). Write metrics now also account for the final batch flushed during task commit, so `recordsWritten` no longer undercounts by the per-task remainder.
