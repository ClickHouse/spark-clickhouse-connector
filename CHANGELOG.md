...
### Fixed
- Sort rows by shard number instead of by sharding key value when writing to a Distributed table with `spark.clickhouse.write.distributed.convertLocal=true` ([#XXX]), on Spark profiles 3.4/3.5/4.0 (Spark 3.3 cannot resolve catalog functions in write sort orders). Consecutive sharding key values map to different shards, so the previous key-value sort forced the writer to flush a tiny batch at almost every distinct key boundary, producing roughly one INSERT per distinct sharding key value instead of about one INSERT per shard per Spark partition. Writes through the session catalog-less `TableProvider` path (`format("clickhouse")`) keep the previous key-value sort, since Spark cannot resolve the shard-number function there.
