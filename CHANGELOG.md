...
### Added
- Anonymous usage statistics via Scarf ([#576](https://github.com/ClickHouse/spark-clickhouse-connector/pull/576)). The connector now sends a lightweight, anonymous usage event to ClickHouse's Scarf endpoint when Spark plans a read from ClickHouse or executes a write to it (at most once per scan or write), carrying only the connector and Spark versions, the OS name, coarse runtime facts (platform, table engine, wire format, test-run flag), a one-way hash of the Spark application name, and a truncated random table UUID — never data, names, hostnames, or credentials. Events are sent asynchronously and best-effort, so they can never fail or slow down a job. Opt out with the new SQL config `spark.clickhouse.sendAnonymousUsageStats=false` (default `true`) or the standard `SCARF_NO_ANALYTICS`/`DO_NOT_TRACK` environment variables. Applies to all Spark profiles (3.3/3.4/3.5/4.0).
