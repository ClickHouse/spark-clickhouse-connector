...
### Fixed
- Stopped leaking the connector-only `ssl` option into the clickhouse-java client ([#563](https://github.com/ClickHouse/spark-clickhouse-connector/pull/563)). The connector consumes `option.ssl` itself to select the `https://`/`http://` URL scheme but also forwarded it to the v2 client, which logged `Unknown and unmapped config properties: {ssl=true}` on every client build. Connector-consumed keys are now stripped from the options passed to the client. No behavior change — SSL is still driven by the URL scheme.
