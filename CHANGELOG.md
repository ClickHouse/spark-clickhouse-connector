...
### Fixed
- Reads no longer silently drop rows when the partition listing is incomplete ([#580](https://github.com/ClickHouse/spark-clickhouse-connector/pull/580)). A scan plans one input partition per partition returned by `system.parts`, each filtering on `_partition_id`, so the rows a scan returns are the union of those filters and a listing that misses a partition prunes it away with no error. `system.parts` is only eventually consistent on ClickHouse Cloud, where each operation opens its own connection and may be served by a replica that has not caught up: two listings of the same table one second apart were observed returning 3 and 4 partitions, and the read built from the shorter one returned 3 of 4 rows. Scans now plan one additional input partition filtering `_partition_id NOT IN (<listed ids>)`, so the filters cover the whole table by construction and an incomplete listing costs read parallelism instead of rows. It is skipped when a scan already reads the whole table and when `spark.clickhouse.read.splitByPartitionId=false`. Applies to all Spark profiles (3.3/3.4/3.5/4.0).
