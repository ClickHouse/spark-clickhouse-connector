...
### Fixed
- A pushed-down aggregate over no matching rows now follows SQL semantics ([#581](https://github.com/ClickHouse/spark-clickhouse-connector/pull/581)). ClickHouse returns a non-nullable column's default rather than NULL for an aggregate over no rows, so a task whose filters matched nothing contributed a `0` that Spark folded into the final value: `SELECT MIN(id) FROM t WHERE id > 999` returned `0` instead of NULL. `MIN`, `MAX` and `SUM` are now pushed down as `minOrNull`, `maxOrNull` and `sumOrNull`. `COUNT` keeps its plain form, since `0` is its correct empty-set value and Spark merges it with a `Sum` whose output attribute is non-nullable, so a NULL there would corrupt the result. All Spark profiles.
- Partition listings are unioned across a table's replicas ([#581](https://github.com/ClickHouse/spark-clickhouse-connector/pull/581)). A scan plans one input partition per partition reported by `system.parts`, each filtering on `_partition_id`, so a listing that omits a partition prunes it out of the scan and silently drops its rows. `system.parts` reflects only the server answering the query, and on an eventually consistent service such as ClickHouse Cloud — where each operation may reach a different replica — it lags a recent write: two listings one second apart returned 3 and 4 partitions, and the read built from the shorter one returned 3 of 4 rows. Whole-table listings now read `clusterAllReplicas(<cluster>, system.parts)`, deduplicated by part name. The cluster is discovered from `system.clusters` — the one containing the answering server with the fewest shards, then the most members — and single-server clusters are ignored, so a non-replicated deployment pays nothing. Where permitted, an unreachable replica is skipped and the union is time-bounded with `timeout_overflow_mode='throw'` pinned, so a listing hitting that bound fails and falls back rather than being read as complete; settings a `readonly=1` user may not change are probed once. Any other failure falls back to the answering server's own view. Only scans filtering by `_partition_id` union, since `spark.clickhouse.read.splitByPartitionId=false` filters on a partition value another replica may render differently; per-shard listings under `read.distributed.convertLocal` are left alone because the union spans every shard, and partition-identifier listings because they consume only that value. `spark.clickhouse.read.partitionListing.unionReplicas=false` turns it off. This narrows the staleness window rather than closing it: a task still reads from whichever server its connection reaches. All Spark profiles.
