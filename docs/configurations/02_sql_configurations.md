...
spark.clickhouse.write.server_settings.<name>|<undefined>|ClickHouse server setting applied when writing. Replace `<name>` with the setting name, for example `spark.clickhouse.write.server_settings.insert_deduplication_token`. Writer options override SQLConf entries with the same setting name.|0.10.1
<!--end-include-->
