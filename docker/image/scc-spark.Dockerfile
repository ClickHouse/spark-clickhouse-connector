...
RUN set -x && \
    ...
    wget -q https://repository.apache.org/content/repositories/snapshots/org/apache/kyuubi/${TPCDS_CONNECTOR_JAR_NAME}/1.6.0-SNAPSHOT/${TPCDS_CONNECTOR_JAR_NAME}-1.6.0-20220802.001957-213.jar -P ${SPARK_HOME}/jars && \
    TPCH_CONNECTOR_JAR_NAME=kyuubi-spark-connector-tpch_${SCALA_BINARY_VERSION} && \
    wget -q https://repository.apache.org/content/repositories/snapshots/org/apache/kyuubi/${TPCH_CONNECTOR_JAR_NAME}/1.6.0-SNAPSHOT/${TPCH_CONNECTOR_JAR_NAME}-1.6.0-20220802.001957-176.jar -P ${SPARK_HOME}/jars && \
