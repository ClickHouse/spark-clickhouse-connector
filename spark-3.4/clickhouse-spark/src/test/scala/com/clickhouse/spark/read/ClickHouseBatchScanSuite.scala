...
class ClickHouseBatchScanSuite extends AnyFunSuite with Log4j2CaptureHelper {
  ...
  test("the complement input partition covers every partition absent from the listing") {
    val complement = inputPartition(NoPartitionSpec, complementOf = Some(Seq("20220411", "20220412")))
  ...
  test("no complement is planned when the listing collapsed to a whole-table scan") {
    ...
    // NoPartitionSpec is an object extending the PartitionSpec case class, so equality is
    // structural: the guard recognises any spec carrying the sentinel's values, not just the
    // singleton instance
    assert(PartitionSpec("", "", 0, 0) === NoPartitionSpec)
  }
